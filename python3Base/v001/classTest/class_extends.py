...
# 判断一个变量是否是某个类型可以用isinstance()判断一个变量是否是某个类型可以用isinstance
...
# 多态的好处就是，当我们需要传入Dog、Cat、Tortoise……时，我们只需要接收Animal类型就可以了，因为Dog、Cat都是Animal类型
...
# 而具体调用的run()方法是作用在Animal、Dog、Cat对象上，由运行时该对象的确切类型决定，这就是多态真正的威力：
